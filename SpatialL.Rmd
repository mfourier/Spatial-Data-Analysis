...
En el contexto de datos espaciales [@8444678], trabajamos con un conjunto de observaciones de datos asociados con una componente espacial $\{(x(s_i),y(s_i)) | \ i \in \mathbb{N} , 1 \leq i \leq n\}$ donde $n \in \mathbb{N}$ es la cantidad de muestras, $s_i \in \mathbb{R}^{2}$ es un vector de coordenadas espaciales, $x(s_i) \in \mathbb{R}^m$ son las variables explicativas del modelo y $y(s_i) \in \mathbb{R}$ es la respuesta a dichas variables.
...
El problema de la predicción consiste en que, dado un conjunto de muestras de data espacial, buscamos modelar una función $f$ tal que $Y = f(X)$, una vez que el modelo se entrena, puede usarse para precedir la respuesta $y$ en otra locación dado las variables explicativas.
...
```{r, results='hide', warning=FALSE, message=FALSE}
# Obtiene acs data y transforma a tipo NYC EPSG
nyc_data <- get_acs(
  geography = "tract",
...
$\textbf{Nota}:$ Se eligió eliminar la variable median_year_built que es utilizada para inferencia en Analyzing US Census Data pues esta posee alrededor de 800 filas en las que toma valor cero entre 2100, se probó el modelo usando alrededor de 1100 datos con esta variable limpia, y por otro lado se probó con los 1967 datos sin hacer uso de esta variable. Los resultados que se adecuan más a la teoría expuesta cuando no se toma en cuenta esta variable median_year_built debido a la alta pérdida de información, y al bajo p-valor que toma esta variable en los posteriores modelos.
