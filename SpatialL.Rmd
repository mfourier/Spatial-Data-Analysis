...
#Al parecer, entregan malos test debido a valor 0 en columna median_year_built

#Despues de realizar pruebas, se confirma que esta columna bugueada ensucia la data
#nyc_census_data_prepped <- filter(nyc_census_data_prepped, median_year_built != 0)
#Probamos manteniendo mas data pero eliminando las variables median_year_built
nyc_census_data_prepped <- nyc_census_data_prepped[ , !names(nyc_census_data_prepped) %in% 
    c("median_year_built","median_structure_age")]
```

\newpage

$\textbf{Nota}:$ La variable median_year_built posee alrededor de 800 filas en las que toma valor cero, esto ensucia la data median_structure_age, pues hace que permita que existan viviendas con tiempo promedio de construcción de 40 años, como viviendas con tiempo promedio de construccion de 2000 años, lo que es absurdo, se probó el modelo usando alrededor de 1100 datos con esta variable limpia, y por otro lado se probó con los 1967 datos sin hacer uso de esta variable. Los resultados que se adecuan más a la teoría expuesta en Analyzing US Census se dieron cuando no se toma en cuenta esta variable median_year_built y median_structure_age.

```{r, echo = FALSE, fig.align = 'center', out.width="100%"}
knitr::include_graphics("image/median_year.png")
```

En las siguientes secciones se obtienen resultados distintos que en [Spatial Machine Learning - J. Morgan](https://justinmorganwilliams.medium.com/spatial-machine-learning-29137dcd1f5f) debido a este cambio, pues trabajaremos con menos datos, pero con una variable que no hace ruido en el modelo.

En efecto sucede que para ciertos test, nos entrega mejores resultados respecto de la autocorrelación en los modelos lagsar(), cuando se ejecutó, daremos un comentario de este punto en dicha sección.
...
#Quitamos median_structure_age del modelo
# compute model
...
Definimos Spatial data como un conjunto de observaciones de data espacial (Jiang Z., 2019) [@8444678] $$\{(x(s_i),y(s_i)) | \ i \in \mathbb{N} , 1 \leq i \leq n\}$$ donde $n \in \mathbb{N}$ es la cantidad de muestras, $s_i \in \mathbb{R}^{2 \times 1}$ es un vector de coordenadas espaciales, $x(s_i) \in \mathbb{R}^m$ son las variables explicativas del modelo y $y(s_i) \in \mathbb{R}$ es la respuesta a dichas variables donde nuestra intención es predecir $y$.
...
Para trabajar el problema de la autocorrelación espacial [@8444678] [@nikparvar2021] hacemos uso del índice de Moran el cual el cual nos entrega una medida de como se comporta este fenómeno dado n observaciones de una variable, el índice de Moran es una auto-covarianza espacial estandarizada [@chen2013].
...
-   El número promedio de conexiones entre distritos censales, el promedio de cuantas conexiones posee cada distrito es $5.43$

-   Existen 5 distritos que no se conectan con nadie.

\newpage

Podemos hacer un plot de la estructura que posee el objeto "neighborhood list", para visualizarlo usamos funciones "st\_" descritas en Analyzing US Census.
...
knitr::include_graphics("image/si_neighborhood_rook_queen.jpg")
```
...
$\textbf{Comentario:}$ Notamos los mismos valores de $R^2$ (a excepción del Modelo 1 por muy poco ) que aquellos replicados en [Spatial Machine Learning - J. Morgan](https://justinmorganwilliams.medium.com/spatial-machine-learning-29137dcd1f5f) cuando lo trabajamos sin tomar en cuenta la variable median_year_built
...
# define formula for grf, quitamos median_structure_age
...
nyc_census_data_prepped_sp$grf <- grf_model$LGofFit$LM_Rsq100 #local r2 grf 


nyc_grf <- tm_shape(nyc_census_data_prepped_sp) +
  tm_fill("grf", 
          palette = "GnBu", 
          title = "Local r2") +
...
```{r, echo = FALSE, fig.align = 'center', out.width="100%"}
knitr::include_graphics("image/localR2GWM.png")
```

Podemos ver que la versión GWR del paquete GWmodel tiene, con diferencia, las puntuaciones R2 locales más altas
