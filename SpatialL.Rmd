...
Se busca predecir precio de viviendas, suponemos que existe autocorrelación espacial sobre esta variable, pues aquellas viviendas que se encuentran cerca, comparten su entorno, y por tanto una serie de factores relacionados al consumo.
...
Se toman datos georreferenciados con nivel geográfico de distritos para la ciudad de NY con nivel geográfico de distritos para la ciudad de NY, para ello pedimos datos de los condados de: 'Bronx', 'Kings', 'New York', 'Queens', 'Richmond'
...
-   **median_value** : El valor medio de la vivienda del distrito censal, nuestra variable a predecir

-   median_rooms : Cantidad media de habitaciones por casa en el distrito censal

-   total_population : Población total en el distrito censal

-   median_age : Edad media de la población en el distrito censal

-   median_year_built : Mediana del año en que se construyó la vivienda

-   median_income : Ingreso medio de los hogares en el distrito censal
...
-   Podemos suponer que $w_{ii} = 0 \ \ \forall i \in \{1, \dots, n\}$, pues el hecho de que un punto tenga influencia con el mismo puede generar ruido, sin embargo esto dependera del modelo
...
$$Y_i = \alpha + \rho Y_{lag-i} + \sum_{k} \beta_k X_{ik} + \varepsilon_i \ \ \forall i \in \{1,...,n\}$$
donde 
...
$$Y_i = \alpha + \sum_{k=1}^{m} \beta_{k} X_{ik} + \varepsilon_i$$
...
El coeficiente $\beta(s_h)$ en locación $s_h$ puede entrenarse vía método de mínimos cuadrados ponderados, minimizando un error ponderado, donde el peso que se le asigna a cada muestra depende de su distancia a $s_h$, es decir, buscamos encontrar $\beta(s_h)$ tal que 
...
-   $\textbf{Teorema:}$ Sea $A \in S^N$ una matriz simétrica, luego sus valores propios $(\lambda_j)_{j=1}^{n}$ son números reales, además
$$\max_{x \in \mathbb{R}^n} \frac{ \langle  \; x, Ax\; \rangle}{||x||^2} = \max_{||x||=1} x^{T}Ax = \lambda_{max}(A) \ \text{y se alcanza en} \ x = v_{max} \ \text{vector propio asociado a} \ \lambda_{max}$$
$$\min_{x \in \mathbb{R}^n} \frac{ \langle  \; x, Ax\; \rangle}{||x||^2} = \min_{||x||=1} x^{T}Ax = \lambda_{min}(A) \ \text{y se alcanza en} \ x = v_{min} \ \text{vector propio asociado a} \ \lambda_{min}$$

El resultado anterior es conocido por cocientes de Rayleigh, para una demostración detallada, ver Teorema 55 en el siguiente documento [The Rayleigh's principle and the minimax principle for the eigenvalues of a self-adjoint matrix](https://people.math.osu.edu/costin.10/5102/Rayleigh%20quotient.pdf)

\newpage

Además, hacemos uso un teorema para estimar cotas sobre los valores propios de la matriz, para ello necesitamos la siguiente definición:

- $\textbf{Definición:}$ (Radio espectral) Sean $(\lambda_{i})_{i=1}^{n}$ los valores propios de una matriz $A \in \mathbb{C}^{n \times n}$, definimos el $\textbf{radio espectral}$ de $A$ como
$$\rho(A) = \max \{ |\lambda_1|, |\lambda_2|,...|\lambda_n|\}$$

-   $\textbf{Teorema:}$ (Desigualdad de Perron-Frobenius) Para una matriz $A$ con entradas no negativas, se cumple que
$$\min_{i} \sum_{j=1}^{n} A_{ij} \leq \rho(A) \leq \max_{i} \sum_{j=1}^{n} A_{ij}$$
Para más detalle, ver Teoremas de Perron-Frobenius, punto 1 en sección *positive matrices* y sección *Inequalities for Perron–Frobenius eigenvalue* [(enlace aquí)](https://en.wikipedia.org/wiki/Perron%E2%80%93Frobenius_theorem)
...
Entonces, usando que $||z||_{2} = 1$ y como $I= n (z^TVz)$
$$n\lambda_{min}(V) = n\min_{||z||=1} z^{T}Vz \leq I  \leq n \max_{||z||=1} z^{T}Vz = n\lambda_{max}(V)$$
$$\implies |I| \leq n \max \{|\lambda_{max}(V)|, |\lambda_{min}(V)|\} \leq n\rho(V)$$
Para encontrar el valor de $\rho(V)$ usamos que $V$ es matriz de pesos estandarizada, tenemos por la desigualdad de Perron-Frobenius

$$\min_{i} \sum_{j=1}^{n} v_{ij} \leq \rho(V) \leq \max_{i} \sum_{j=1}^{n} v_{ij}$$

Como $\sum_{i=1}^{n} \sum_{j=1}^{n} v_{ij} = 1$ y $V$ es $\textit{row-normalized}$, es decir, cada fila suma la misma cantidad, tenemos $\sum_{j=1}^{n} v_{ij} = \frac{1}{n} \ \ \forall i \in \{1,\dots,n\}$, por lo que en particular
$$\min_{i} \sum_{j=1}^{n} v_{ij} = \max_{i} \sum_{j=1}^{n} v_{ij} = \frac{1}{n}$$
Por lo tanto
$$\min_{i} \sum_{j=1}^{n} v_{ij} \leq \rho(V) \leq \max_{i} \sum_{j=1}^{n} v_{ij}$$
$$ \iff \frac{1}{n} \leq \rho(V) \leq \frac{1}{n} $$
$$\implies \rho(V) = \frac{1}{n}$$
Luego, retomando la desigualdad del índice de Moran, podemos deducir que

$$|I| \leq n\rho(V) = n \times \frac{1}{n} = 1$$
$$\implies I \in [-1,1]$$
...
$$\mathbb{E}_{\pi}(I) = \mathbb{E}_{\pi} (\frac{N}{W} \frac{\sum_{i=1}^{N} \sum_{j=1}^{N} w_{ij} ((\pi x)_i - \pi \bar{x})((\pi x)_j - \pi \bar{x})}{\sum _{i=1}^{N} ((\pi x)_i - \pi \bar{x})^2})$$

Claramente, $\pi \bar{x} = \bar{x}$, además haciendo la suma sobre $i$ tenemos

$$\sum _{i=1}^{N} ((\pi x)_i - \pi \bar{x})^2 = \sum _{i=1}^{N} (x_i -  \bar{x})^2$$
Luego, usando la linealidad de la esperanza

$$= \frac{N}{W} \frac{\sum_{i=1}^{N} \sum_{j=1}^{N} w_{ij} \mathbb{E}_{\pi}(((\pi x)_i - \pi \bar{x})((\pi x)_j - \pi \bar{x})))}{\sum _{i=1}^{N} (x_i -  \bar{x})^2}$$
...
$$ = \sum_{\pi_0 \in S^N} f(\pi_0) \frac{1}{N!} = \frac{1}{N!} \sum_{\pi_0 \in S^N} ((\pi_{0} x)_i - \bar{x})((\pi_0 x)_j - \bar{x})$$

Como $\pi_0$ es una permutación, entonces $(\pi_0 x)_i \neq (\pi_0 x)_j$ para $i \neq j$ y además notamos que para $(x_i,x_j)$ fijos existen $(N-2)!$ permutaciones $\pi$ tales que $(x_i,x_j) = ((\pi x)_i, (\pi x)_j)$ para $i \neq j$, por lo tanto

$$\sum_{\pi_0 \in S^N} ((\pi_0 x)_i - \bar{x})((\pi_0 x)_j - \bar{x}) = (N-2)!\sum_{i \neq j} (x_i - \bar{x})(x_j - \bar{x}) $$
...
$$\sum_{i \neq j} (x_i - \bar{x})(x_j - \bar{x}) = \sum_{i,j} (x_i - \bar{x})(x_j - \bar{x}) - \sum_{i = j} (x_i - \bar{x})(x_j - \bar{x})$$
$$= (\sum_{i} (x_i - \bar{x}) \sum_{j}(x_j - \bar{x})) - \sum_{k} (x_k - \bar{x})(x_k - \bar{x})$$
$$= (\sum_{k} x_k - \bar{x} )^2 - \sum_{k} (x_k - \bar{x})^2$$

Por lo tanto

$$ \mathbb{E}_{\pi}[((\pi x)_i - \pi \bar{x})((\pi x)_j - \pi \bar{x})] = \frac{1}{N(N-1)} [ (\sum_{k} x_k - \bar{x})^2 - \sum_{k} (x_k - \bar{x})^2]$$
Y entonces 
$$ = \frac{-\sum_{k}(x_k - \bar{x})^2}{N(N-1)}$$ 
Luego
$$\mathbb{E}_{\pi}(I) = \frac{N}{W} \frac{\sum_{i=1}^{n} \sum_{j=1}^{n} w_{ij} (\frac{-\sum_{k}(x_k - \bar{x})^2}{N(N-1)})}{\sum _{i=1}^{n} (x_i -  \bar{x})^2}$$ 
De donde, simplificando términos, deducimos que
