...
Lo definimos como un conjunto de observaciones de data espacial (Jiang Z., 2019) [@8444678] 
$$\{(x(s_i),y(s_i)) | \ i \in \mathbb{N} , 1 \leq i \leq n\}$$ 
...
Definimos el índice de Moran como [@8444678]
$$I = \frac{\sum_{i=1}^{n} \sum_{j=1}^{n} W_{ij} (y(s_i) - \bar{y})(y(s_j) - \bar{y})}{(\sum_{i=1}^{n} \sum_{j=1}^{n} W_{ij}) \sum _{i=1}^{n} (y(s_i) - \bar{y}) /n }$$
O de manera equivalente

$$= \frac{n}{W}\frac{\sum_{i=1}^{n} \sum_{j=1}^{n} W_{ij} (y(s_i) - \bar{y})(y(s_j) - \bar{y})}{\sum _{i=1}^{n} (y(s_i) - \bar{y})}$$
donde $\bar{y} = \sum_{i=1}^{n} y(s_i)/n$ con $n$ el número total de muestras, $W = \sum_{i=1}^{n} \sum_{j=1}^{n} W_{ij}$.
