...
El problema de la predicción consiste en que, dado un conjunto de muestras de data espacial, buscamos modelar una función $f$ tal que $Y = f(X)$, una vez que el modelo se entrena, puede usarse para precedir la respuesta en otra locación dado las variables explicativas.
...
Importamos datos con $\textit{tidycensus}$ provenientes de la American Community Survey (ACS) 5 year (2016-2020), haciendo uso de la función get_acs()
...
Las variables que importamos, y las cuales usaremos como regresores para los modelos de predicción, son las siguientes:

-   median_value : El valor medio de la vivienda del tramo censal
...
Los detalles para descargar información de las distintas bases de datos que proporciona $\textit{tidycensus}$ y las distintas variables que tenemos a disposición se encuentran en el $\textbf{Anexo 3}$.
...
$$ Y_i = \alpha \sum_{k} \beta_{k}X_{ik} + u_i$$
donde 
